...
public class HbaseDataSyncEsObserver extends BaseRegionObserver {
    ...
    private static void readConfiguration(CoprocessorEnvironment env) {
        ...
        ESClient.isFieldContainsFamily = conf.getBoolean("is_family", false);
    }
    ...
    public void start(CoprocessorEnvironment e) throws IOException {
        ...
        LOG.info("------observer init EsClient ------" + ESClient.getInfo());
    }
        ...
        try {
            ...
            for (Map.Entry<byte[], List<Cell>> entry : familyMap.entrySet()) {
                for (Cell cell : entry.getValue()) {
                    ...
                    String esFiled;
                    if(ESClient.isFieldContainsFamily) esFiled = family + ":" + qualifier;
                    else esFiled = qualifier;
                    json.put(esFiled, value);
                }
            }
            ElasticSearchBulkOperator
                    .addUpdateBuilderToBulk(ESClient.client.prepareUpdate(ESClient.indexName, ESClient.typeName, indexId)
